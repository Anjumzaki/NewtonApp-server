...
app.delete("/delete/blog/:id", (req, res) => {
  Blog.findOne({ _id: req.params.id }).then(blog => {
    blog
      .remove()
      .then(() => res.json({ success: true, message: "blog deleted" }));
  });
});
...
app.put("/changePassword", function(req, res) {
  ...
  User.findOne({ _id: req.body.uid }).then(response => {
    console.log("check user", response);
...
app.get("/get/all/transactions/yearly/:uid/:year", (req, res) => {
  ...
  Transaction.find({
    ...
    soldDate: { $regex: `.*${req.params.year}.*` }
  })
    .then(data => {
      res.json(data);
    })
...
app.post("/delete/trasc/:id", function(req, res) {
  ...
  Transaction.findByIdAndRemove(req.params.id, function(err, output) {
    if (err) {
      res.send(err);
    }
...
app.post("/edit/trasc/:id", function(req, res) {
  console.log("bonus", req.body);
  Transaction.updateOne(
    { _id: req.params.id },
    {
